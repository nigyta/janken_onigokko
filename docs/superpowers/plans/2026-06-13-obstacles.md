...
└── test.js      … テスト11件追加(25 → 36件。うち1件はレビュー指摘のトンネリング防止)
```
...
> **レビュー追補:** 高速エージェント(speed ≥ 400 px/秒)が薄い障害物を1フレームで飛び越える
> トンネリングを防ぐため、個人速度を `MAX_SPEED = 350 px/秒` でクランプし(1フレーム最大移動量
> 35px < 障害物最小40px)、専用テストを1件追加した(35 → 36件)。
...
3. Run: `node test.js` → Expected: `# pass 36` `# fail 0`
